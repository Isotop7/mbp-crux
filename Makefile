...
ISO_PACKAGES      = filesystem bash dash coreutils bzip2 e2fsprogs jfsutils \
                    ...
                    elfutils linux-pam libaio json-c f2fs-tools inih
...
kernel/linux-$(KERNEL_VERSION).tar.xz:
	cd kernel && wget $(KERNEL_MIRROR)/pub/linux/kernel/v5.x/linux-$(KERNEL_VERSION).tar.xz
...
initramfs: kernel busybox initramfs/init | rootfs
	...
	@sed -i -e "s/size=.*m/size=$(shell bsdtar -tvf rootfs/rootfs.tar.xz | awk '{ size += $$5 } END { printf "%.0f\n",size/1024/1024*1.03 }')m/" initramfs/init
...
iso: image-base memtest syslinux
	...
	@cp -a /usr/lib/grub/fonts $(IMG_ROOT_DIR)/boot/grub/
	@cp iso/grub.cfg $(IMG_ROOT_DIR)/boot/grub/
	@cp iso/shell.efi $(IMG_ROOT_DIR)/boot/
